...
class CWTHDF5Dataset(Dataset):
    def __init__(
        ...
        split_col = (
            "trace_dataset_split"
            if "trace_dataset_split" in self.meta.columns
            else "split"
        )
        ...
        if max_distance is not None:
            ...
            if dist_col:
                self.meta = self.meta[
                    (self.meta[dist_col] <= max_distance) | self.meta[dist_col].isna()
                ]
    ...
    def __getitem__(self, idx):
        ...
        p_col = (
            ...
            if "trace_p_arrival_sample" in row
            else "trace_P_arrival_sample"
        )
        s_col = (
            "trace_s_arrival_sample"
            if "trace_s_arrival_sample" in row
