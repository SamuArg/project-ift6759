...
class Sta_Lta:
    ...
    def pick_trace(self, trace, metadata, s_pick, verbose):
        ...
        min_length = int(
            max(self.lta_window_sec_p, self.lta_window_sec_s) * self.sampling_rate
        )
...
def test_picker():
    ...
    try:
        p_pick, s_pick = picker.pick_trace(
            trace=waveforms, metadata=metadata, s_pick=True, verbose=True
        )
