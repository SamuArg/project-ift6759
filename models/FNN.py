...
def cwt_transform(traces, sf, down_sample_factor, cwt_widths=None):
    ...
    for i in range(num_channels):
        ...
        cwt_mat, freqs = cwt(
            traces[i], scales=cwt_widths, wavelet=wavelet, sampling_period=1 / sf
        )
...
modelUnetSimple = CWTUNetPhasePicker(
    in_channels=3, base_channels=16, use_coords=False, coord_channels=3, simple=True
)
modelUnetDouble = CWTUNetPhasePicker(
    in_channels=3, base_channels=16, use_coords=False, coord_channels=3, simple=False
)
total_params = sum(p.numel() for p in modelUnetDouble.parameters() if p.requires_grad)
print(f"Total trainable parameters: {total_params:,}")
dummy_spectrogram = torch.randn(
    1, 3, 129, 60
)  # Batch=1, Channels=3, Freq=129, Time=120
