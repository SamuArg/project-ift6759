    ...
    rows.append(
        {
            ...
            "type": "Model+Coords",
            "size": size,
            "MAE": stead_model_coords["MAE"][i],
            "R2": stead_model_coords["R2"][i],
        }
    )
    rows.append(
        {
            ...
            "type": "Model",
            "size": size,
            "MAE": instance_model["MAE"][i],
            "R2": instance_model["R2"][i],
        }
    )
    rows.append(
        {
            ...
            "type": "Model+Coords",
            "size": size,
            "MAE": instance_model_coords["MAE"][i],
            "R2": instance_model_coords["R2"][i],
        }
    )
...
for dataset in datasets:
    for metric in metrics:
        ...
        ax.scatter(
            ...
            model[metric],
            marker="o",
            color=palette[0] if dataset == "STEAD" else palette[1],
            label="Model",
        )
        ax.scatter(
            coords["size"],
            ...
            marker="o",
            facecolors="white",
            edgecolors=palette[0] if dataset == "STEAD" else palette[1],
            label="Model + Coords",
        )
        ...
        for size in input_sizes:
            ...
            if not base.empty and not coord.empty:
                ax.plot(
                    [size, size],
                    [base[metric].values[0], coord[metric].values[0]],
                    linestyle="--",
                    color=palette[0] if dataset == "STEAD" else palette[1],
                    alpha=0.7,
                )
        ...
        baseline = (
            stead_baseline[metric] if dataset == "STEAD" else instance_baseline[metric]
        )
        ax.hlines(
            y=baseline,
            xmin=min(input_sizes) - 5,
            xmax=max(input_sizes) + 5,
            colors="gray",
            linestyles=":",
            label="Baseline",
        )
