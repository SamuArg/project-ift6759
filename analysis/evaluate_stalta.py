...
def _isolated_ar_pick_worker(args):
    ...
    try:
        ...
        picker = Sta_Lta(
            sta=kwargs['sta'], lta=kwargs['lta'], sampling_rate=kwargs['sampling_rate'],
            low_band_pass_freq=kwargs['low_band_pass_freq'], upper_band_pass_freq=kwargs['upper_band_pass_freq'],
            num_ar_p=kwargs['num_ar_p'], num_ar_s=kwargs['num_ar_s'],
            var_window_length_p=kwargs['var_window_length_p'], var_window_length_s=kwargs['var_window_length_s']
        )
        
        return picker.pick_trace(trace=trace, metadata=metadata, s_pick=True, verbose=False)
    finally:
...
def evaluate_stalta(dataset_name="INSTANCE", fraction=1.0, tolerance=0.1):
    ...
    if 0.0 < fraction < 1.0:
        ...
        print(f"Subsampled {fraction*100:.1f}% of the test set: {num_samples} events selected.")
    ...
    s_col = "trace_s_arrival_sample" if "trace_s_arrival_sample" in meta.columns else "trace_S_arrival_sample"
    ...
    for idx in tqdm(range(len(sb_dataset))):
        ...
        gt_p_sec = float(true_p_sample) / sampling_rate if not np.isnan(true_p_sample) else float("nan")
        gt_s_sec = float(true_s_sample) / sampling_rate if not np.isnan(true_s_sample) else float("nan")
        ...
        config = {
            'sta': 0.2, 'lta': 2.0, 'sampling_rate': sampling_rate,
            'low_band_pass_freq': 1.0, 'upper_band_pass_freq': 45.0,
            'num_ar_p': 2, 'num_ar_s': 2,
            'var_window_length_p': 0.2, 'var_window_length_s': 0.2
        }
         
        try:
            future = pool.submit(_isolated_ar_pick_worker, (waveforms, metadata, config))
            p_pick, s_pick = future.result()
            
            if p_pick < 0: p_pick = float("nan")
            if s_pick < 0: s_pick = float("nan")
    ...
    metrics = evaluate_seismic_detection(predictions, ground_truth, tolerance=tolerance, magnitudes=magnitudes)
