...
def plot_predictions(model, plot_loader, device, n=8,
                     confidence_thr=0.3, sampling_rate=100, out_path=None):
    """
    ...
    if not collected:
        print("No earthquake traces found in the sampled batches — try increasing batch size.")
        return
    ...
    fig.suptitle("Sample Predictions  (— P prob   — S prob   ‒ ‒ ground truth)",
                 fontsize=13, y=1.01)
    ...
    for idx, item in enumerate(collected):
        ...
        ax_pp = fig.add_subplot(rows * 2, cols, (2 * row_block + 1) * cols + col_pos + 1)
        ...
        ax_w.set_title(
            f"Trace {idx + 1}  P={p_t:.1f}s  S={s_t:.1f}s",
            fontsize=8
        )
        ...
        ax_pp.axhline(confidence_thr, color="gray", ls=":", lw=0.9,
...
if __name__ == "__main__":
    ...
    test_pipe   = SeisBenchPipelineWrapper(
        dataset_name=DATASET, split="test", model_type=pipeline_type,
        transformation_shape="gaussian", transformation_sigma=10,
    )
